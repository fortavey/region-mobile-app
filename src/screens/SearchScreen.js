...
export const SearchScreen = ({ navigation }) => {
  ...
  let currentRegion = (currentRegion = fullListArr.filter((el) =>
    el.code.includes(parseInt(number))
  ))
    ...
    if (currentRegion.length) {
      return (
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('Item', { ...currentRegion })}
        >
  ...
  useEffect(() => {
    if (number.length) {
      currentRegion.length
        ? setRender(currentRegion[0].name)
        : setRender('Ничего не найдено')
    } else {
      setRender('')
    }
  })
  ...
  const pressNumber = (num) =>
    number.length < 3 ? setNumber(number + num) : null
  ...
  return (
    <View style={styles.search}>
      ...
      <View style={styles.result}>
        <Text
          style={{ fontSize: 30, color: THEME.MAIN_COLOR, textAlign: 'center' }}
        >
          {render}
        </Text>
      </View>
